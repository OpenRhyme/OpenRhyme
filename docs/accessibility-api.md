...
- Working pattern: **one dedicated capture thread** that owns a `RunLoop`. Create observers and the tap there, add their sources there, spin the loop there. Model it as a custom `@globalActor` (`AXActor`) whose executor dispatches onto that thread, so every AX/CG call is isolated to it and the compiler enforces it. Hand results outward through an `AsyncStream<RawEvent>`, where `RawEvent` is a plain `Sendable` struct (pid, bundle id, kind, strings, timestamps) — never an `AXUIElement`.
