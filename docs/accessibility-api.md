...
**Privacy (as shipped)** (`Sources/Capture/PrivacyPolicy.swift`, `SecretRedactor.swift`, `Redaction.swift`, `AXClient.swift`): the never-capture policy is evaluated *inside* `focusedContext`, between the cheap identity read (bundle id, window title, document, URL) and the expensive content ladder above — so a protected context is never read into the process at all, not read then discarded. A protected row (`extra.protected: true`, `extra.protectedBy: "<rule>"`) carries only `pid`/`bundle_id`/`app_name`; no window title, document, URL, element, value, or selected text. With no focused window at all, the URL/document/window-title rules cannot be evaluated, so the context is treated as protected (`"unverifiable-context"`) rather than falling through to a real read — the same guard covers `inspect`'s equivalent call. Text that does get captured passes through `Redaction.apply`, the single chokepoint every captured value already goes through: secure-field guard → credential-field guard (an element whose identifier/title looks like `password`/`token`/`api_key`/… is treated exactly like `AXSecureTextField`) → the existing byte cap → `SecretRedactor`'s corpus (structural rules plus a high-entropy backstop) over what's left, reported as `extra.redacted`. `openrhyme events`/`export` re-apply `SecretRedactor` at read time across every text-bearing column, so a rule added after capture still cleans up what was already stored.

**Noise reduction (as shipped)** (`Sources/Capture/TitleNormalizer.swift`, `Fingerprint.swift`, `HeartbeatDiff.swift`, `Capturer.swift`): change detection compares two identities, not raw strings — `extra.valueHash` (SHA-256 hex of the redacted value) and `extra.fingerprint` (the first 16 hex characters of SHA-256 over `bundleID ␟ normalized window title ␟ document ␟ URL-without-fragment`, place-level, a contract for downstream grouping — sessions and the semantic layer). Titles are compared after `TitleNormalizer` (rule table: leading `(N) ` notification counters, status glyphs, Chrome `Audio playing` / `Muted` / `High memory usage` tab badges, whitespace collapse — stored titles stay raw). Every observer notification is classified by `ax.secondsSinceLastInput()` as `user` (≤ `capture.user_input_window_seconds`) or `ambient`, recorded in `extra.input`; ambient title and value notifications are dropped (the heartbeat samples them instead) and user-driven titles debounce the same way values do. Readout roles (`AXSlider`, `AXProgressIndicator`, `AXValueIndicator`, `AXScrollBar`) never yield a value. Anonymous elements — no title, identifier, value, or selected text, and not secure — are transparent to change detection: the element fields of the signature carry forward from the previous state. A value whose hash was stored within `capture.content_memory_seconds` (32 hashes kept per pid) is emitted as `valueUnchanged: true` with no body. App activations are read after `capture.activation_settle_ms` so the focused window and element agree; keep `activation_settle_ms` ≤ `value_debounce_ms` (defaults 200 ms ≤ 500 ms satisfy it) — a debounced title/value refresh scheduled inside the settle window would otherwise fire before the AX tree has settled, the exact window/element mis-pairing the settle exists to prevent. The observed notification set is configurable (`capture.notifications`, `capture.apps.<bundle-id>.notifications`; `value` implies `focus`), and a config edit re-registers the affected observers within one heartbeat.
...
6. **Emit** to `Store` — one `events` table, no tiers. Sessions and summaries are built downstream (the semantic layer design); `Capture` knows nothing about them.
