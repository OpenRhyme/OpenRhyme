...
- `protectedURLPatterns`: `1password.com`, `bitwarden.com`, `lastpass.com`, `dashlane.com`, `/ui/vault/`, `://vault.`, `/settings/credentials`, `/trust-credentials`, `/admin/credentials`, `/iam-admin/serviceaccounts`, `/apikeys`.
  **As shipped, corrected from the design (Task 1, Task 12 docs pass):** the bare form `vault.` false-positives on a domain like `evault.com` (`evault.com` contains the substring `vault.`) — the shipped pattern is `://vault.`, anchored to the scheme separator, which `evault.com` does not contain. `console.cloud.google.com/iam-admin/serviceaccounts` was also shipped as the domain-independent `/iam-admin/serviceaccounts`, deliberately broader (any IAM service-account console, not only Google's).
- `protectedDocumentPatterns`: `.env`, `.env.*`, `*.pem`, `*.key`, `*.p12`, `*.keystore`, `id_rsa*`, `id_ed25519*`, `id_ecdsa*`, `*credentials*`, `*secrets*`, `.npmrc`, `.netrc`, `.pgpass`, `*/.aws/*`, `*/.ssh/*`, `*/.gnupg/*`.
  **As shipped, corrected from the design:** `credentials`/`credentials.*`/`secrets.*` (whole-component patterns) missed real filenames like `aws-credentials.json` or `prod-secrets-2024.yaml` — a genuine miss in the slice's core purpose (Task 1 review, Ruling R4). Shipped as the affix-matching `*credentials*`/`*secrets*`, and every document pattern match is case-insensitive (`FNM_CASEFOLD`), so `.ENV`/`Secrets.yaml` are also caught.
- `protectedWindowTitlePatterns`: `private browsing` (Safari's private windows carry it).
- `credentialFieldPatterns`: `password`, `passwd`, `secret`, `token`, `api key`, `api_key`, `apikey`, `private key`, `passphrase`, `otp`, `2fa`, `mfa code`. (The design listed `current-password`/`new-password` separately; shipped without them since substring matching on `password` already covers both — no behaviour difference.)
...
Signature becomes `apply(_ element: ElementInfo?, maxValueBytes: Int, policy: PrivacyPolicy) -> RedactedText`, and `RedactedText` gains `redactedRules: [String]`. Order: secure-field guard (unchanged) → **credential-field guard** (§5.1, same outcome as secure) → byte cap → secret redaction. The single existing call site in `HeartbeatDiff` is the only caller.
**As shipped, corrected from the design:** the design listed a separate `entropyEnabled: Bool` parameter on `apply` itself; the shipped signature has no such parameter — it reads `policy.entropyRedaction` internally and passes that through to `SecretRedactor.redact(_:entropyEnabled:)` (§5.2's own function, which does keep its own `entropyEnabled` parameter). One less way for a caller to pass a policy and a stale entropy flag that disagree.
...
```
`FocusedContext` gains `protection: Protection` (default `.open`). `inspect` respects the policy by default: run against a protected context it prints the rule that protected it and nothing else. `openrhyme inspect --ignore-privacy` overrides this for debugging, prints a warning to stderr first, and never writes to the store. An unconditional exemption would have made `inspect` a bypass of the guarantee this slice exists to make.

**As shipped, added beyond the design (Task 4+5 review, Rulings R6/R9/R22):** the design's "nothing else in the AX layer changes" is no longer accurate. `AXClient.focusedContext` and its `inspect`-only sibling `focusedElementInspection` both gained a windowless guard, `PrivacyPolicy.protectionForWindowlessContext()`: when `AXFocusedWindow` returns `nil` and any URL/document/window-title pattern is configured, the URL/document/window-title rules cannot be evaluated at all, so the context is treated as `.protected(rule: "unverifiable-context")` rather than falling through to a real content read. This closed a real gap — a protected page reachable only through the focused element (no readable window) would otherwise have been harvested, and with `url`/`document` both `nil` on the row `purge --apply-rules` could never have found it retroactively either. Gated on `policy.enabled`, so `privacy.enabled: false` and `inspect --ignore-privacy` (which passes `.disabled`) still see everything, as intended.
...
- **`openrhyme events`** — re-applies `SecretRedactor` at read time and adds `--max-value-chars` (moved from the MCP, `0` = full). Rows already stored redacted are unaffected (idempotent).
  **As shipped, corrected from the design (Ruling R32, Task 10 fix round 1):** `--max-value-chars` defaults to `0` (full text), not `2000`. The flag did not exist before this slice — `openrhyme events` always returned full values — so a `2000` default would have silently truncated a public output contract (`openrhyme … --json`) for every existing caller, which the project's own non-negotiables forbid without a version bump and migration. The flag is additive and opt-in; the Python MCP, which did default to 2000 before this slice, keeps doing so by passing the flag explicitly. Also broader than described: read-time redaction covers all six text-bearing columns (`value`, `selected_text`, `window_title`, `url`, `document`, `element_title`), not just `value`/`selected_text` — a credential in a URL query string or a window title is just as real a leak (privacy fix round 1, J8/R27) — plus `extra.previousTitle`, the one `extra` key that ever carries verbatim captured text (privacy fix round 3, S3).
...
- `PrivacyPolicyTests`: each default list matches its intended cases and, importantly, its intended *non*-cases (`environment.md` is not `.env`; `mykey.pem` matches, `keynote.app` does not; `://vault.` matches `https://vault.corp.com/...` but not `https://evault.com/...` — anchored to the scheme separator so a bare `vault.` false-positive on `evault.com` cannot happen); `add`/`remove` config semantics; empty policy protects nothing.
