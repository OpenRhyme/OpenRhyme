...
**Status:** proposed. Depends on Phase 2 (semantic consolidation) for entity extraction.
...
Edge table in `semantic.sqlite`. Two edge types:
...
1. **Entity resolution:** merge aliases (fuzzy name match + embedding similarity on entity descriptions). Example: "RunaxAI scheduler" and "scheduler" and "the scheduler module" → one canonical entity with aliases `["RunaxAI scheduler", "scheduler", "the scheduler module"]`.
...
| File | What | Lines |
|---|---|---|
| `src/openrhyme_mcp/graph.py` | Entity resolution, edge derivation, BFS traversal | ~180 |
| `src/openrhyme_mcp/server.py` | `graph_traverse()` tool | ~30 |
| `tests/test_graph.py` | Fixture entities/edges, test BFS, test resolution | ~100 |
...
5. No engine modifications needed.
