...
- **Pattern detection is best-effort.** The redactor matches known secret shapes everywhere, and adds a Shannon-entropy backstop on field content only; it will miss custom or unusual secret formats, and a shapeless secret in a URL or window title is not caught at all. Treat it as a safety net, not a guarantee.
