...
```
 ...
 └──────────────────────────────────┬──────────────────────────────────────────┘
                                    │  `openrhyme … --json` only (redacted on read)
                                    ▼
...
- **One source of truth.** No WARM/COLD storage tiers. SQLite handles ~100K events/day with no pressure. A companion semantic store is built asynchronously by a local LLM — rebuildable from the raw events, and deleted alongside them (rebuildable is a recovery story, not a deletion one).
- Session boundaries follow **activity coherence** (idle-gap detection), not fixed time windows.
- **No bundled LLM, and no inference in this repo at all.** The capture daemon never calls a model. LLM work happens in a separate consolidation worker (launchd agent, no TCC grants, Python repo) that reads through the CLI — so everything it sees has already been redacted, same as every other consumer.
...
- **What about a secret pasted into an ordinary window** (terminal, editor, browser form)? Recognised secret shapes (AWS/GitHub/Stripe/Slack/Google/OpenAI/Anthropic keys, private-key blocks, JWTs, `key=value` secrets, connection-string passwords) are redacted to `[redacted:<rule>]` before the row is written — in all six text columns (`value`, `selected_text`, `window_title`, `url`, `document`, `element_title`) plus `extra.previousTitle`, so a key in a URL query string is redacted on disk too, not just on the way out. The high-entropy-token backstop is deliberately narrower: it runs on `value` and `selected_text` only, because its "20+ characters, mixed case, a digit" test also describes every opaque document id, and shredding the id in a Google Docs or Notion URL would destroy the only thing recording *which document you were reading* while protecting nothing (see Limits). The same pass runs again every time a row is read back, so a rule added today also cleans up what was captured last week (see Limits: that read-time pass is the *only* thing protecting rows captured before the rule existed — those still hold the plaintext on disk). This is best-effort pattern matching, not a guarantee (see Limits).
...
| `privacy.entropy_redaction` | `true` | the Shannon-entropy backstop that catches high-entropy tokens with no matching structural rule. Applies to `value`/`selected_text` only; the structural rules apply to every text column. |
...
Sixteen of them, stated plainly. If a claim above sounds stronger than a limit here, the limit is the accurate one.
...
- **The entropy backstop does not run on URLs, document paths, or titles.** It catches unrecognised secret shapes in `value` and `selected_text` only. On identifying columns it was shredding opaque resource ids — a Google Docs or Notion id, a Dropbox or S3 key — which are names, not credentials, and which are the whole reason the URL is worth storing. The trade is explicit: a credential in a URL is caught only if it matches one of the structural rules (which is what `?token=…`, `AKIA…`, `ghp_…`, `sk_live_…`, a JWT and a connection-string password all do). A house-format secret with no recognised shape, sitting in a URL or a window title, is stored as-is — protect that context with a `protected_url_patterns` rule instead.
...
The capture daemon and MCP server are built. **None of the semantic layer below is implemented yet** — there is no `semantic.sqlite` on your disk today, and everything `openrhyme purge` and the retention sweep promise applies to the one store that exists.
...
| **0.5** | Derived-data deletion — `purge`/retention clean the companion store, `privacy`/`status` report it | None | Deleting an event deletes what was derived from it |
...
Three commitments the design makes up front, because they are the ones worth breaking the phase order for:

- **Phase 0.5 gates everything that writes.** A summary reading *"logged into 1Password, entry github.com (work)"* outliving the purge of the events it came from would be the same failure this project already fixed five times during the privacy slice. So the engine learns to delete from the companion store *before* anything learns to write to it, and a purge that cannot clean it says so instead of reporting success.
- **The companion store gets the same lock as the events store** — `0600` in a `0700` directory, corrected on existing files. A summary distils what raw rows only imply; it is the more sensitive of the two, not the less.
- **The consolidation worker reads through `openrhyme events`**, so everything it sees is already redacted, and it never passes `--ignore-privacy`. Its model runs locally by default. Pointing it at a hosted provider is an explicit opt-in that sends your activity summaries off the machine — and nothing in OpenRhyme can delete them once it has.
