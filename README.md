...
- **How do I verify what's actually happening?** `openrhyme privacy` — prints the policy in force, evaluates the frontmost app/window live, and counts how many stored rows the current rules would protect.
...
- **Permission hardening (`0700`/`0600`) only runs when the daemon starts.** An install from before this slice stays at the old, looser permissions until the daemon is restarted once — restart it (`openrhyme daemon`, or however your launchd/service manager runs it) to pick up the tightened permissions.
- **Read-time redaction invalidates `extra.valueHash`** for any row it alters — the stored hash was computed over the original, unredacted text, so it no longer matches what `openrhyme events`/`export` return for that row. A mismatch there is actually a useful signal: it means capture-time redaction missed something a later rule caught.
...
- Pattern-based secret detection is best-effort and will miss custom formats; file permissions stop other accounts, not other processes running as you.
