...
@Suite struct AppsCommandTests {
    @Test func allowListDenyRoundTrip() async throws {
        let dir = try CLIRunner.tempDataDir()
        let env = ["OPENRHYME_DATA_DIR": dir.path]

        var result = try await CLIRunner.run(["apps", "list", "--json"], env: env)
        ...
        result = try await CLIRunner.run(
            ["apps", "allow", "com.apple.TextEdit", "--json"], env: env)
        ...
        result = try await CLIRunner.run(["apps", "deny", "com.apple.TextEdit"], env: env)
        #expect(result.status == 0)
    ...
    @Test func allowLeavesAHandWrittenPrivacyBlockByteIdentical() async throws {
        let dir = try CLIRunner.tempDataDir()
    ...
    @Test func allowRejectsObviouslyInvalidIdentifier() async throws {
        let env = ["OPENRHYME_DATA_DIR": try CLIRunner.tempDataDir().path]
        let result = try await CLIRunner.run(["apps", "allow", "TextEdit", "--json"], env: env)
